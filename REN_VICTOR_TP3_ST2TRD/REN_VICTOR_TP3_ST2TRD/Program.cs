...
namespace REN_VICTOR_TP3_ST2TRD
{
    class MovieCollection
    {
        static void Main(string[] args)
        {
            ...
            var all =
                (from m in movie.Movies
                 select m).Count();
            Console.WriteLine("There is {0} movie in the collection\n", all);

            Console.WriteLine("\nQ3 : \n");
            var e =
                (from m in movie.Movies
                 where m.Title.Contains("e") == true
                 select m).Count();
            Console.WriteLine("There is {0} movie in the collection with the letter 'e' at least once in the titre \n", e);

            Console.WriteLine("\nQ4 : \n");
            var f =
                (from m in movie.Movies
                 select m.Title.Count(f => f == 'f')).Sum();
            Console.WriteLine("There is{0} time the letter 'f' in all tities \n", f);
            ...
            var HighBudget =
                (from m in movie.Movies
                 ...
                 select m).First();
            Console.WriteLine("{0},  the movie with the highest budget (${1})\n", HighBudget.Title, HighBudget.Budget);

            Console.WriteLine("\nQ6 : \n");
            var lowBox =
                (from m in movie.Movies
                 orderby m.BoxOffice ascending
                 where (m.BoxOffice != null && m.BoxOffice > 1000)
                 select m).First();
            Console.WriteLine("{0}, is the movie with the lowest box office (${1})\n", lowBox.Title, lowBox.BoxOffice);

            Console.WriteLine("\nQ7: \n");
            var onze =
                (from m in movie.Movies
                 orderby m.Title ascending
                 select m.Title).Take(11);
            int i = 0;
            Console.WriteLine("The 11 first movie titles are : \n");
            foreach (string m in onze)
            {
            ...
            var av1980 =
                (from m in movie.Movies
                 where m.ReleaseDate < new DateTime(1980, 01, 01)
                 select m).Count();
            ...
            var avgRunningTime =
                (from m in movie.Movies
                 where vowels.Contains(m.Title[0])
                 select m.RunningTime).Average();
            ...
            var letter =
                from m in movie.Movies
                where ((m.Title.ToLower().Contains('h') || m.Title.ToLower().Contains('w')) && (m.Title.ToLower().Contains('i') == false || m.Title.ToLower().Contains('t') == false))
                select m.Title;

            Console.WriteLine("The movies that contain the letter 'H' OR 'W' but not the letters 'I' OR 'T' : \n");
            i = 0;
            foreach (string m in letter)
            {
                Console.WriteLine("{0} - {1}\n", i, m);
                i++;
            }
            ...
            var meanBudget =
                (from m in movie.Movies
                 where (m.BoxOffice != null && m.Budget != null && (m.Budget / m.BoxOffice) < 50)
                 select (m.Budget / m.BoxOffice)).Average();

            Console.WriteLine("Mean of all Budget / Box Office of every movie ever is {0})", meanBudget);

           


            Exo2 exercice2 = new Exo2();
            exercice2.ThreadStar();
            Console.WriteLine("End EXO 2");
        ...
        public class Exo2
        {
            ...
            public void ThreadStar()
            {
                ...
                Thread empty = new Thread(new ThreadStart(() => ThreadProc(' ', 50)));
                empty.Start();
                empty.Join(10000);

                Thread star = new Thread(new ThreadStart(() => ThreadProc('*', 40)));
                star.Start();
                star.Join(11000);
                ...
                static void ThreadProc(char a, double period)
                {
                    ...
                    var periodTimeSpan = TimeSpan.FromMilliseconds(period);
